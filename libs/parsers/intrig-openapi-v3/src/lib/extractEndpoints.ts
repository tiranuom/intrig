import { Controller, Endpoint, HttpMethod } from '@tiranuom/intrig-common';
...
export function extractRequestBody(
  ...
  const camelCasePath = capitalizeFirstLetter(
    path.replace(/[-_]([a-z])/g, (g) => g[1].toUpperCase())
  );
  const camelCaseMethod = capitalizeFirstLetter(
    method.replace(/[-_]([a-z])/g, (g) => g[1].toUpperCase())
  );
  ...
  for (const contentType in operation.requestBody.content) {
    if (operation.requestBody.content.hasOwnProperty(contentType)) {
      ...
      if (schema?.hasOwnProperty('$ref')) {
        requestBodyType = capitalizeFirstLetter(
          schema['$ref'].split('/').pop()
        );
      } else {
        // If the schema is not a reference, use your schema naming strategy
        requestBodyType = capitalizeFirstLetter(
          `${camelCasePath}${camelCaseMethod}RequestBody`
        );
      }
  ...
  for (const contentType in successResponse.content) {
    if (successResponse.content.hasOwnProperty(contentType)) {
      ...
      if (schemaObj && schemaObj.hasOwnProperty('$ref')) {
        responseType = capitalizeFirstLetter(
          schemaObj['$ref'].split('/').pop()
        );
      } else {
...
export function extractErrorResponses(
  ...
  for (const responseCode in operation.responses) {
    if (
      operation.responses.hasOwnProperty(responseCode) &&
      (responseCode.startsWith('4') || responseCode.startsWith('5'))
    ) {
      const errorResponse = operation.responses[responseCode];

      let camelCasePath = capitalizeFirstLetter(
        path.replace(/[-_]([a-z])/g, (g) => g[1].toUpperCase())
      );
      let camelCaseMethod = capitalizeFirstLetter(
        method.replace(/[-_]([a-z])/g, (g) => g[1].toUpperCase())
      );
      ...
      for (const contentType in errorResponse.content) {
        if (errorResponse.content.hasOwnProperty(contentType)) {
          ...
          if (schemaObj && schemaObj.hasOwnProperty('$ref')) {
            errorResponseType = capitalizeFirstLetter(
              schemaObj['$ref'].split('/').pop()
            );
          } else {
