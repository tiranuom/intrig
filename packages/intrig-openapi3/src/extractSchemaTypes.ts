...
function isOpenApiV3Document(object: any): boolean {
  return object && object.hasOwnProperty('openapi') && object.openapi.startsWith('3.') && object.hasOwnProperty('info') && object.hasOwnProperty('paths');
}
    ...
    for(const method in openApiDoc.paths[path]) {
      ...
      for(const response in operationObj.responses) {
        if(operationObj.responses[response].content) {
          for(const mediaType in operationObj.responses[response].content) {
            const schema = operationObj.responses[response].content[mediaType].schema;
            if(schema) {
              const schemaName = capitalizeFirstLetter(toCamelCase(`${cleanPath}_${method}_${response}`));
      ...
      if(operationObj.requestBody && operationObj.requestBody.content) {
        for(const mediaType in operationObj.requestBody.content) {
          ...
          if(schema) {
            const schemaName = capitalizeFirstLetter(toCamelCase(`${cleanPath}_${method}_requestBody`));
            schemas[schemaName] = schema;
          }
        }
      }
      ...
      (operationObj.parameters || []).forEach((param: any, index: number) => {
        if(param.schema) {
          const schemaName = capitalizeFirstLetter(toCamelCase(`${cleanPath}_${method}_param_${index}`));
