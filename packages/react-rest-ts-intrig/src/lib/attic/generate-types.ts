...
export function generateInterfaces(schema: Schema, name: string): string {
  ...
  const properties = Object.entries(schema.properties || {}).map(
    ([propertyName, propertyType]) => {
      ...
      if (
        schema.required &&
        (schema.required as string[]).includes(propertyName)
      ) {
        required = '?';
      }
      if (propertyType.$ref) {
  ...
  let interfaceResult = interfaceTemplate({
    interfaceName: name,
    properties,
    description,
    createdDate,
...
export async function generateAndSaveInterfaces(
  ...
  return Promise.all(
    Object.entries(schemas).map(async ([name, schema]) => {
      ...
      console.log(
        `Interface for ${name} has been saved to ${interfaceFileName}`
      );
    })
  );
}
