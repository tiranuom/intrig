...
export function collectRefs(schema: Schema, path: string = ''): string[] {
  ...
  if (schema.hasOwnProperty('properties')) {
    Object.keys(schema['properties']).forEach((propertyKey) => {
      ...
      if (typeof propertyValue === 'object' && propertyValue !== null) {
        refs = refs.concat(
          collectRefs(propertyValue, `${path}/${propertyKey}`)
        );
      }
    });
  }
  ...
  if (schema.hasOwnProperty('items')) {
    ...
    if (typeof schema['items'] === 'object' && schema['items'] !== null) {
      refs = refs.concat(
        collectRefs(schema['items'] as Schema, `${path}/items`)
      );
    }
  }
...
export function resolveDefinitions(
  ...
  collector: { [key: string]: Schema } = {}
) {
  let refs = collectRefs(schema).reduce((acc: { [k: string]: Schema }, cur) => {
