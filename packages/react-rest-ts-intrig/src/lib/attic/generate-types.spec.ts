...
export interface MyInterface {
    ...
    expect(generateInterfaces(schema, 'MyInterface').trim()).toEqual(
      expectedOutput
    );
  });
...
export interface MyArrayInterface {
    ...
    expect(generateInterfaces(schemaArray, 'MyArrayInterface').trim()).toEqual(
      expectedArrayOutput
    );
  });
...
export interface MyNestedInterfaceNestedProp {
    ...
    expect(
      generateInterfaces(schemaNested, 'MyNestedInterface').trim()
    ).toEqual(expectedNestedOutput);
  });
...
export interface MyRefInterface {
    ...
    expect(generateInterfaces(schemaRef, 'MyRefInterface').trim()).toEqual(
      expectedRefOutput
    );
  });
});
