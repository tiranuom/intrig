...
const NetworkStateContext = createContext<NetworkContextType | undefined>(undefined);

function networkStateReducer(state: Record<string, NetworkState<unknown>>, action: { key: string, state: NetworkState<unknown> }) {
...
export function NetworkProvider({ children }: { children: ReactNode }) {
    ...
    const setNetworkState = (key: string, state: NetworkState<unknown>) => dispatch({ key, state });
