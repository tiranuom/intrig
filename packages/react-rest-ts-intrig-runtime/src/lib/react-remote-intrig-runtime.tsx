 ...
 */
export function useCallIntegration<T>(
  requestMetadata: NetworkRequestMetadata
): [NetworkState<T> | null, (request: NetworkRequest<T>) => void, () => void] {
  const getNetworkState = useGetNetworkState();
  const setNetworkState = useSetNetworkState();
  const axios = useIntrig();

  const currentKey = useMemo(
    () => generateRequestKey(requestMetadata),
    [requestMetadata]
  );
  ...
  const resetNetworkState = () =>
    setNetworkState(currentKey, { status: 'idle' });

  return [
    getNetworkState(currentKey) as NetworkState<T>,
    makeRequest,
    resetNetworkState,
  ];
}
...
export function useResetNetworkState() {
  ...
  if (setNetworkState === undefined) {
    throw new Error(
      'useResetNetworkState must be used within a NetworkProvider'
    );
  }
