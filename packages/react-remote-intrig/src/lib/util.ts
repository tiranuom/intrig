...
// Usage: {{snakeCase myString}}
    handlebars.registerHelper('snakeCase', function(str: string): string {
        return str.replace(/[\s-]+([a-zA-Z])/g, (match, letter) => `_${letter.toLowerCase()}`);
    });
...
// Usage: {{substring myString start end}}
    handlebars.registerHelper('substring', function(str: string, start: number, end: number): string {
        return str.substring(start, end);
    });

// Replace occurrences of a substring
// Usage: {{replace myString find replaceWith}}
    handlebars.registerHelper('replace', function(str: string, find: string, replaceWith: string): string {
        return str.replace(new RegExp(find, 'g'), replaceWith);
    });

    handlebars.registerHelper('interpolateUrl', function(path: string, context: any): string {
