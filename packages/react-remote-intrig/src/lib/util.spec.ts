...
describe('Template helpers', () => {
    ...
    it('camelCase helper', () => {
        const camelCaseTemplate = handlebars.compile('{{camelCase (removeWhitespace str)}}');
    ...
    it('replace helper', () => {
        const replaceTemplate = handlebars.compile('{{replace str find replaceWith}}');
        expect(replaceTemplate(context)).toBe('my Awesome String');
    });

    it('Interpolate url helper', () => {
        const replaceTemplate = handlebars.compile('{{interpolateUrl url "pathVariable"}}');
        expect(replaceTemplate(context)).toBe("/pet/${encodeURIComponent(pathVariables.petId)}/value");
    });
});
