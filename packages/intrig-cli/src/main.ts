...
import {Config} from "../../intrig-common/src/main";
...
yargs
  .scriptName('intrig')
  .command('init', 'Creates intrig.config.json file', (yargs) =>
      yargs.option('type', {
        type: 'string',
    ...
    async (argv) => {
      ...
      if (!configType || !validTypes.includes(configType)) {
        if(configType){
          console.log(`Invalid value provided for the 'type' option: ${configType}`);
        }
      ...
      if (!language || !['ts', 'js', 'dart'].includes(language)) {
        if(language){
          console.log(`Invalid value provided for the 'lang' option: ${language}`);
        }
        const responses: any = await inquirer.prompt([{
          type: 'list',
          name: 'lang',
  ...
  .command('add', 'Adds a new source to sources array',
    (yargs) => yargs.option('sourceType', {
      type: 'string',
      description: 'Source type to add',
      choices: Object.keys(sourceTypes)
    }),
    async (argv) => {
      const configData = await fs.promises.readFile(configPath, {encoding: 'utf-8'});
      const config: Config = JSON.parse(configData);

      let sourceTypeName = argv.sourceType as string;
      if (!sourceTypeName || !Object.keys(sourceTypes).includes(sourceTypeName)) {
        if (sourceTypeName) {
          console.log(`Invalid value provided for the 'sourceType' option: ${sourceTypeName}`);
        }
        const responses: any = await inquirer.prompt([{
          type: 'list',
          name: 'sourceType',
  ...
  .command('remove', 'Removes a source from sources array',
    ...
    async (argv) => {
      ...
      if (!sourceName || !config.sources.map(s => s.name).includes(sourceName)) {
        if (sourceName) {
          console.log(`Invalid value provided for the 'name' option: ${sourceName}`);
        }
        const responses: any = await inquirer.prompt([{
          type: 'list',
          name: 'name',
  ...
  .command('generate', 'Generates the code based on configuration', {},
    async () => {
